...
//                if([faces count] != 0)
//                    tempCountFrame ++; //Giảm thời gian trả về frame. 2 Frame mới trả về 1 lần, tránh nóng máy khi base64 nhiểu
                if([faces count] != 0){ // && tempCountFrame % 2 == 0
//                    if( tempCountFrame == 10000)
//                        tempCountFrame = 0;
                    //---------Xử lý cắt Ảnh hình Vuông-----
                    float perDownSize = 1;
                    int newSize = image.size.width;
                    ...
                    //--resize
                    CGFloat scale = MAX(sizeNew.width/image.size.width, sizeNew.height/image.size.height);
                    CGFloat width = image.size.width * scale;
                    CGFloat height = image.size.height * scale;
                    CGRect imageRect = CGRectMake((sizeNew.width - width)/2.0f,
                                                (sizeNew.height - height)/2.0f,
                                                width,
                                                height);
                    //--Giam Size
                    UIGraphicsBeginImageContextWithOptions(sizeNew, NO, perDownSize);
                    [image drawInRect:imageRect];
                    UIImage *destImage = UIGraphicsGetImageFromCurrentImageContext();
                    UIGraphicsEndImageContext();
                    //---------END Xử lý cắt Ảnh hình Vuông-----
                   
                    //---------Parse base 64-----
                    //NSData *imageData = UIImagePNGRepresentation(destImage);
                    NSData *imageData = UIImageJPEGRepresentation(destImage, 0.2); // Giảm dung lượng ảnh.
                    base64String = [imageData base64EncodedStringWithOptions:0];
                                //NSLog(@"XXX sizeIMG: %fx%f", image.size.width, image.size.height);
                                //NSLog(@"XXX sizeIMG_New: %fx%f", destImage.size.width, destImage.size.height);
//                                NSLog(@"XXX scale: %i", tempCountFrame);
//                                NSLog(@"XXX size: %lu", [base64String length]);
                }
                //---------BẮN FRAME base 64 lên Code ReactNative-----
                NSDictionary * eventFace = @{@"type" : @"face", @"frame" : base64String, @"faces" : faces};
